...
class MoodleService:
    ...
    async def get_course_completion_statuses(self, course_id: int) -> List[Dict]:
        if not self.moodle_token:
            logger.warning("Moodle token not configured, returning empty list")
            return []
        ...
        for user in users:
            ...
            try:
                form_data = FormData()
                form_data.add_field('wstoken', self.moodle_token)
                form_data.add_field('wsfunction', 'core_completion_get_course_completion_status')
                form_data.add_field('moodlewsrestformat', 'json')
                form_data.add_field('courseid', str(course_id))
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        content_type = resp.headers.get('Content-Type', '')
                        logger.info(
                            "Moodle completion response -> user=%s status=%s reason=%s content_type=%s",
                            user_id, resp.status, resp.reason, content_type
                        )
                        raw = await resp.text()
                        logger.info(
                            "Moodle completion raw body (user=%s, first 500 chars, len=%s): %s",
                            user_id, len(raw), raw[:500]
                        )

                        if resp.status != 200:
                            logger.warning(f"Moodle API returned status {resp.status} for user {user_id}")
                            continue

                        if 'json' not in content_type.lower():
                            logger.warning(
                                "Unexpected content-type for completion status (expected json) for user %s: %s",
                        ...
                        if isinstance(data, dict) and 'exception' in data:
                            logger.warning(f"Moodle API error for user {user_id}: {data.get('message', 'Unknown error')}")
                            continue
                        
                        completionstatus = data.get('completionstatus', {})
                        completed = completionstatus.get('completed', False) if isinstance(completionstatus, dict) else False
                ...
                logger.error(f"Error fetching completion status for user {user_id}: {e}", exc_info=True)
                continue
